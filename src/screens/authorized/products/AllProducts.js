...
const ListHeader = ({ title, color }) => {
    return (
        <View style={{ backgroundColor: priColor, flexDirection: 'row', paddingHorizontal: 10, alignItems: 'center', paddingVertical: 15, }}>
            ...
            <Text style={{ color: 'white', fontSize: responsiveFontSize(1.9), fontWeight: 'bold' }}>{title.toUpperCase()}</Text>
        </View>
    );
}
...
class AllProducts extends Component {
    ...
    componentDidMount() {
        this.setState({
            ...
            fetch(`${host}/products?per_page=12&page=${this.state.current_page}`)
                .then(res => res.json())
                .then(resData => {
                    // console.log(resData);
                    if (resData.code === 200) {
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={{ flex: 1, backgroundColor: 'white' }}>
                    <ListHeader title='Tất cả sản phẩm' color='blue' />
