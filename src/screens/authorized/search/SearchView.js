...
class SearchView extends Component {
    ...
    onSearch = () => {
        fetch(`${host}/scan-code`, {
            ...
            body: JSON.stringify({
                'code': this.state.searchKey.toString()
            })
        })
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <SearchHeader onChangeText={(text) => this.onChangeText(text)} onSearch={() => this.onSearch()} navigation={this.props.navigation} />
                <View style={{ flex: 1, backgroundColor: priColor }}>
                    {JSON.stringify(this.state.data) !== JSON.stringify({}) ?
                        ...
                        :
                        <Text style={{ textAlign: 'justify', fontSize: responsiveFontSize(2), color: 'white', padding: 15 }}>Mời bạn nhập mã code để tìm kiếm thông tin sản phẩm</Text>
                    }
                </View>
            </View >
        );
